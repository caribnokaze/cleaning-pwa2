...
共通パスワードは初回認証時だけ入力します。ログイン成功後は入力値をメモリから消去し、
端末へ保存しません。期限のない端末専用トークンをiOS Keychain／Android Keystoreで保護された
SecureStoreへ保存し、アプリ再起動時はログイン状態を復元します。ログアウト時はサーバー側でも
トークンを無効化し、端末変更時は新しい端末で再認証します。5回連続失敗時のログイン制限は
10分です。Web版は従来どおり12時間で認証期限が切れます。

ログイン後、担当者と現場の選択肢は認証済みの`/api/mobile/report-options`から取得します。
このAPIはWeb版の報告フォームと同じ一覧を返します。モバイルでは番号・氏名または現場名で
検索して候補から選択し、一覧にない自由入力値では写真カテゴリーへ進めません。送信APIも
同じ一覧で再検証し、選択肢外の担当者・現場を拒否します。
